-- Generamos el esquema y las tablas

create schema keepmoving;

create table keepmoving.coche(
...
create table keepmoving.modelo(
	id serial primary key,
	id_marca INT not null,
	nombre VARCHAR(50) not null
);

create table keepmoving.marca(
	id serial primary key,
	id_grupo INT not null,
	nombre VARCHAR(50) not null
);

create table keepmoving.grupo(
	id serial primary key,
	nombre VARCHAR(50) not null
);

create table keepmoving.color(
	id serial primary key,
	nombre VARCHAR(20) not null
);

create table keepmoving.revision(
	id serial primary key,
	id_coche VARCHAR(17) not null,
	fecha DATE not null,
...
create table keepmoving.poliza(
	n_poliza INT4 primary key,
	id_coche VARCHAR(7) not null,
	id_aseguradora INT not null,
	fecha_alta DATE not null
);

create table keepmoving.aseguradora(
	id serial primary key,
	nombre VARCHAR(25) not null
);

create table keepmoving.divisa(
	id serial primary key,
...
alter table keepmoving.coche add constraint pk_modelo_coche foreign key (id_modelo) references keepmoving.modelo(id);

alter table keepmoving.coche add constraint pk_color_coche foreign key (id_color) references keepmoving.color(id);

alter table keepmoving.modelo add constraint pk_marca_modelo foreign key (id_marca) references keepmoving.marca(id);

alter table keepmoving.marca add constraint pk_grupo_marca foreign key (id_grupo) references keepmoving.grupo(id);

alter table keepmoving.revision add constraint pk_coche_revision foreign key (id_coche) references keepmoving.coche(matricula);

alter table keepmoving.revision add constraint pk_divisa_revision foreign key (id_divisa) references keepmoving.divisa(id);

alter table keepmoving.poliza add constraint pk_aseguradora_poliza foreign key (id_aseguradora) references keepmoving.aseguradora(id);

alter table keepmoving.poliza add constraint pk_coche_poliza foreign key (id_coche) references keepmoving.coche(matricula);


-- Generamos la tabla auxiliar de datos que usaremos para poblar el esquema con los datos.

create table keepmoving.coches (
...
insert into keepmoving.coches (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) values
...
-- Poblamos las tablas con los datos de la tabla auxiliar "coches"
	
insert into keepmoving.aseguradora (nombre) 
select aseguradora as nombre
from keepmoving.coches
group by aseguradora;

insert into keepmoving.grupo (nombre)
select grupo as nombre
from keepmoving.coches
group by grupo;

insert into keepmoving.divisa (nombre)
select moneda as nombre
from keepmoving.coches
group by moneda;

insert into keepmoving.color (nombre)
select color as nombre
from keepmoving.coches
group by color;

insert into keepmoving.marca (nombre, id_grupo)
select c.marca, g.id
from keepmoving.coches c
inner join keepmoving.grupo g on g.nombre = c.grupo
group by c.marca, g.id;

insert into keepmoving.modelo (id_marca, nombre)
select m.id, c.modelo
from keepmoving.coches c
inner join keepmoving.marca m on c.marca = m.nombre
group by c.modelo, m.id;

insert into keepmoving.coche (matricula, id_modelo, id_color, fecha_compra, kilometraje_total)
select c.matricula, m.id, co.id, c.fecha_compra, c.kms_totales 
from keepmoving.coches c
inner join keepmoving.modelo m on c.modelo = m.nombre
inner join keepmoving.color co on c.color = co.nombre 
group by c.matricula, m.id, co.id, c.fecha_compra, c.kms_totales;

insert into keepmoving.poliza (n_poliza, id_coche, id_aseguradora, fecha_alta)
select c.n_poliza, c.matricula, a.id, c.fecha_alta_seguro
from keepmoving.coches c
inner join keepmoving.aseguradora a on a.nombre = c.aseguradora 
group by c.n_poliza, c.matricula, a.id, c.fecha_alta_seguro;

insert into keepmoving.revision (id_coche, fecha, kilometraje, importe, id_divisa)
select c.matricula, c.fecha_revision, c.kms_revision, c.importe_revision, d.id 
from keepmoving.coches c
inner join keepmoving.divisa d on d.nombre = c.moneda;


-- Eliminamos la tabla auxiliar

drop table keepmoving.coches;


-- Ejecutamos la consulta indicada en la práctica

select
	q1."Modelo",
	q1."Marca",
	q1."Grupo empresarial",
	q1."Fecha de compra",
	q1."Matrícula",
	q1."Color",
	q1."Kilometraje total",
	a.nombre as "Aseguradora",
	p2.n_poliza as "Número de última póliza"
from
	(
	select
		m.nombre as "Modelo",
		...
		c2.nombre as "Color",
		c.kilometraje_total as "Kilometraje total",
		MAX(p.fecha_alta) as "Fecha última póliza"
	from
		keepmoving.coche c
	inner join keepmoving.modelo m on m.id = c.id_modelo
	inner join keepmoving.marca m2 on m2.id = m.id_marca
	inner join keepmoving.grupo g on g.id = m2.id_grupo
	inner join keepmoving.color c2 on c2.id = c.id_color
	inner join keepmoving.poliza p on p.id_coche = c.matricula
	group by
		m.nombre,
		m2.nombre,
		g.nombre,
...
inner join keepmoving.poliza p2 on q1."Matrícula" = p2.id_coche
	and q1."Fecha última póliza" = p2.fecha_alta
inner join keepmoving.aseguradora a on p2.id_aseguradora = a.id;
